...
def set_transaction_type_if_needed(
    ...
    if "type" not in transaction_dict:
        if (
            "gasPrice" in transaction_dict
            and "accessList" in transaction_dict
        ):
...
def transaction_rpc_to_rlp_structure(
    ...
    if access_list:
        ...
        rlp_structured_access_list = _access_list_rpc_to_rlp_structure(
            access_list
        )
        dictionary = assoc(
            dictionary, "accessList", rlp_structured_access_list
        )
    return dictionary
    ...
    for d in access_list:
        ...
        rlp_structured_access_list.append(
            (
                ...
                tuple(
                    _ for _ in d["storageKeys"]
                ),  # tuple of storage key values
            )
        )
...
def transaction_rlp_to_rpc_structure(
    dictionary: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    if access_list:
        dictionary = dissoc(dictionary, "accessList")
        rpc_structured_access_list = _access_list_rlp_to_rpc_structure(
            access_list
        )
        dictionary = assoc(
            dictionary, "accessList", rpc_structured_access_list
        )
    return dictionary
...
def _access_list_rlp_to_rpc_structure(access_list: Sequence) -> Sequence:
    if not is_rlp_structured_access_list(access_list):
        raise ValueError(
            "provided object not formatted as rlp-structured access list"
        )
