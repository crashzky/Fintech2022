    ...
    for struct_name in structured_data["types"]:
        ...
        for field in structured_data["types"][struct_name]:
            ...
            if not re.match(TYPE_REGEX, field["type"]):
                raise ValidationError(
                    "Invalid Type `{0}` in `{1}`".format(
                        field["type"], struct_name
                    )
                )


def validate_field_declared_only_once_in_struct(
    field_name, struct_data, struct_name
):
    if (
        len([field for field in struct_data if field["name"] == field_name])
        != 1
    ):
        raise ValidationError(
...
def validate_EIP712Domain_schema(structured_data):
    ...
    if "EIP712Domain" not in structured_data["types"]:
        raise ValidationError(
            "`EIP712Domain struct` not found in types attribute"
        )
