...
class TypedTransaction:
    ...
    def __init__(
        ...
        if not isinstance(transaction, _TypedTransactionImplementation):
            raise TypeError(
                "expected _TypedTransactionImplementation, got %s"
                % type(transaction)
            )
    ...
    def from_dict(cls, dictionary: Dict[str, Any]):
        ...
        if not (
            "type" in dictionary
            and is_int_or_prefixed_hexstr(dictionary["type"])
        ):
    ...
    def from_bytes(cls, encoded_transaction: HexBytes):
        ...
        if not isinstance(encoded_transaction, HexBytes):
            raise TypeError(
                "expected Hexbytes, got %s" % type(encoded_transaction)
            )
        if not (
            len(encoded_transaction) > 0 and encoded_transaction[0] <= 0x7F
        ):
        ...
        if encoded_transaction[0] == AccessListTransaction.transaction_type:
            ...
            transaction = AccessListTransaction.from_bytes(
                encoded_transaction
            )
            ...
            raise TypeError(
                "typed transaction has unknown type: %s"
                % encoded_transaction[0]
            )
        return cls(
            transaction_type=transaction_type,
...
class AccessListTransaction(_TypedTransactionImplementation):
    ...
    def payload(self) -> bytes:
        ...
        rlp_structured_dict = transaction_rpc_to_rlp_structure(
            self.dictionary
        )
